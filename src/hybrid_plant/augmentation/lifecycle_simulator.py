...
class LifecycleSimulator:
    ...
    def _find_best_k(
        ...
        CUF meets or exceeds ``hard_threshold_cuf``.

        Strategy
        ────────
        1. Iterate k = min_k, min_k+1, …, max_k.
        2. Early-exit at the first k whose post-event CUF >= hard_threshold_cuf.
        ...
        for k in range(self._min_k, self._max_k + 1):
            ...
            # Early exit: minimum k that meets the hard threshold
            ...
            # Loop exhausted max_k without meeting the hard threshold
            warnings.warn(
                f"Augmentation Year {event_year}: k-search exhausted max_k={self._max_k} "
                f"without reaching hard threshold {hard_threshold_cuf:.4f}%. "
