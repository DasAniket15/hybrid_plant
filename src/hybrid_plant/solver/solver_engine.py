...
class SolverResult:
    ...
    augmentation_result:    dict[str, Any] | None = field(default=None)
    """Augmentation-aware finance result dict (populated only by run_augmentation_aware)."""
...
class SolverEngine:
    ...
    # ── Pass-2: Augmentation-aware solver ────────────────────────────────────

    def run_augmentation_aware(
        self,
        ...
        """
        Pass-2 Optuna search: each trial evaluates augmentation lifecycle NPV.

        Uses the same search space as ``run()``, but the objective calls
        ``augmentation_engine.evaluate_scenario(params)`` instead of the
        bare finance pipeline.  The best solution is re-evaluated in
        slow (full re-sim) mode for the dashboard.

        Parameters
        ----------
        augmentation_engine : AugmentationEngine — constructed with Pass-1
                              trigger threshold and shared plant engine
        n_trials            : override solver.yaml n_trials
        n_jobs              : override solver.yaml n_jobs
        show_progress       : display Optuna progress bar

        Returns
        -------
        SolverResult  — ``full_result["finance"]`` contains augmentation data;
                        ``augmentation_result`` mirrors ``full_result["finance"]``
        """
        n_trials = n_trials or self._n_trials
        n_jobs   = n_jobs   or self._n_jobs

        aug_trial_log: list[dict[str, Any]] = []

        def _aug_objective(trial: optuna.Trial) -> float:
            PENALTY = -1e15
            params  = self._suggest(trial)
            try:
                result  = augmentation_engine.evaluate_scenario(
                    params, fast_mode=self._fast_mode
                )
                finance = result["finance"]
                year1   = result["year1"]
                feasible    = self._is_feasible(finance)
                ...
                aug_trial_log.append({
                    ...
                    "lcoe":                 round(finance["lcoe_inr_per_kwh"], 4),
                    "landed_tariff_y1":     round(finance["landed_tariff_series"][0], 4),
                    "n_aug_events":         finance["augmentation"]["n_events"],
                })

                return savings_npv if feasible else PENALTY

            except Exception as exc:
                warnings.warn(f"Aug trial {trial.number} failed: {exc}")
                aug_trial_log.append({
                    "trial_number": trial.number,
                    "feasible":     False,
                    "error":        str(exc),
        ...
        try:
            ...
            if not feasible:
                raise RuntimeError(
                    f"Aug solver interrupted after {completed} trials with no feasible solution."
                ) from None
            print(f"\n  [Aug Solver] Interrupted after {completed} trials — "
                  f"returning best of {len(feasible)} feasible trials.")

        # ── Re-run best in full (slow) mode ───────────────────────────────────
        best_params = {**study.best_trial.params, **self._FUTURE_FIXED}
        aug_result  = augmentation_engine.evaluate_scenario(best_params, fast_mode=False)
        finance     = aug_result["finance"]
        year1       = aug_result["year1"]

        trials_df = pd.DataFrame(aug_trial_log)
        if not trials_df.empty and "savings_npv_cr" in trials_df.columns:
            trials_df = (
                trials_df[trials_df["feasible"] == True]
                .sort_values("savings_npv_cr", ascending=False)
        ...
        full_result = {"year1": year1, "finance": finance}
